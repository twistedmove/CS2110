...
public class GUIButterfly implements ActionListener {
	...
	static int NORTH_INDEX = 0;
	static int EAST_INDEX = 1;
	static int SOUTH_INDEX = 2;
	static int WEST_INDEX = 3;
	...
	/** Initialize the flapping animation frames. */
	...
	/** Initialize a deck of images for a flapping animation in direction d. */
	private BufferedImage[] initFrames(String d) {
		BufferedImage[] frames = new BufferedImage[UNIQUE_FRAMES];
		for (int i = 0; i < UNIQUE_FRAMES; i++) {
			String filename = "res/butterfly/butterfly_" + d;
			filename += "_" + i + ".png";
	...
	/** True iff the butterfly is moving. */
	...
	/** Update the butterfly's position based on the move count and fpm. */
...
//			System.out.println("");
	...
	/** Nudge the butterfly's position along by (dx, dy) */
	public void nudge(float dx, float dy) {
		xTopLeft = Common.mod(xTopLeft + dx, mapWidth);
		yTopLeft = Common.mod(yTopLeft + dy, mapHeight);
	}
	
	/** Return the small amount to nudge the butterfly each turn in the x 
     * direction. */
	private float getNudgeX() {
		return dx / (float)slowfpm;
	}
	
	/** Return the small amount to nudge the butterfly each turn in the y
     * direction. */
	...
	// Helper Methods
	////////////////////////////////////////////////////////////////////////////
	/** Return the top left corner of the butterfly. */
	...
	// Drawing
	////////////////////////////////////////////////////////////////////////////
	/** Draw the butterfly at top-left position (x, y) using graphics g. The 
	...
	/** Return the current frame in the flap animation. */
