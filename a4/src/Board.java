...
public class Board {
    ...
    public void makeMove(Move move) {
    	int column = move.getColumn(); //variable for the column a piece is placed
		if (board[0][column] != null) { //if top row of move's column is full
										//then throw an exception
			throw new IllegalArgumentException();
		}

		for (int x = NUM_ROWS - 1; x >= 0; x--) { //for the move's column the 
			                                     //piece is placed at the most
												//bottom empty row
			if (board[x][column] == null) {
				board[x][column] = move.getPlayer();
				break;
			}
		}
    }
    ...
    public Move[] getPossibleMoves(Player p) {
    	LinkedList<Move> possibleMoves = new LinkedList<Move>(); //linked list to save 
    															//player's possible moves
		if (hasConnectFour() != null) { //if game not over then return new empty move 
				return new Move[] {};
				}
		for (int x = 0; x < NUM_COLS; x++) {
			if (board[0][x] == null) { //if top row of move's column is empty
									   //then add move to possible moves array
				possibleMoves.add(new Move(p, x));
			}
		}
